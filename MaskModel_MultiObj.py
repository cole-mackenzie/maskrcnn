            ...
            with tf.device('/gpu:%d' % i):
                with tf.name_scope('tower_%d' % i):
                    ...
                    zipped_inputs = zip(self.inner_model.input_names,
                                        self.inner_model.inputs)
                    inputs = [
                        layers.Lambda(lambda s: input_slices[name][i],
        ...
        with tf.device('/cpu:0'):
            ...
            for outputs, name in zip(outputs_all, output_names):
                ...
                if K.int_shape(outputs[0]) == ():
                    # Average
                    m = layers.Lambda(lambda o: tf.add_n(o) / len(outputs), name=name)(outputs)
                else:
                    # Concatenate
                    m = layers.Concatenate(axis=0, name=name)(outputs)
                merged.append(m)
        return merged
...
class Dataset(object):
    ...
    def __init__(self, ims, bbox, class_map=None):
        ...
        self.count = len(ims)
        self.height = ims[0].shape[0]
        self.width = ims[0].shape[1]
        self.bboxes = bbox
        ...
        for i in range(self.count):
            self.add_image(
                ...
                imArr=np.asarray(ims[i]), shapes=np.asarray(["word" for w in self.bboxes[i]])
            )
