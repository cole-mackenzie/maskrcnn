...
class Model():
    ...
    def __init__(self, mode, configs, model_dir):

        assert mode in ['training', 'live']
        self.mode = mode
        self.config = configs
    ...
    def build(self):
        ...
        assert self.mode in ['training', 'live']
        ...
        input_image = layers.Input(
                                    shape=[None, None, self.config.IMAGE_SHAPE[2]],
                                    name="input_image"
                                    )
        input_image_meta = layers.Input(
                                        shape=[self.config.IMAGE_META_SIZE],
        ...
        if self.mode == 'training':
            ...
            gt_boxes = layers.Lambda(
                                    lambda x: norm_boxes_graph(
        ...
        P3 = layers.Add(name="fpn_p3add")(
                                            [
                                                ...
                                                layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c3p3')(C3)
                                            ]
                                            )
        P2 = layers.Add(name="fpn_p2add")(
                                             [
                                                layers.UpSampling2D(size=(2, 2), name="fpn_p3upsampled")(P3),
                                                layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c2p2')(C2)
                                               ]
                                            )
        # Attach 3x3 conv to all P layers to get the final feature maps.
        P2 = layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p2")(P2)
        P3 = layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p3")(P3)
        P4 = layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p4")(P4)
        P5 = layers.Conv2D(self.config.TOP_DOWN_PYRAMID_SIZE, (3, 3), padding="SAME", name="fpn_p5")(P5)
        ...
        if self.mode == "training":
            ...
            if not self.config.USE_RPN_ROIS:
                ...
                input_rois = layers.Input(
                                            shape=[self.config.POST_NMS_ROIS_TRAINING, 4],
                                            name="input_roi",
                                            dtype=np.int32
                                            )
                # Normalize coordinates
            ...
            rpn_bbox_loss = layers.Lambda(lambda x: rpn_bbox_loss_graph(self.config, *x), name="rpn_bbox_loss")(
    ...
    def detect(self, images, verbose=0):
        ...
        """
        assert self.mode == "live", "Create model in inference mode."
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    rpn_bbox: [batch, H * W * anchors_per_location, (dy, dx, log(dh), log(dw))] Deltas to be
                applied to anchors.
    """
    input_feature_map = layers.Input(
...
class DetectionLayer(engine.Layer):
...
if __name__ == "__main__":
    ...
    def build_model(x_train, num_classes):
        ...
        inputs = layers.Input(shape=x_train.shape[1:], name="input_image")
        ...
        return models.Model(inputs, x, "digit_classifier_model")
